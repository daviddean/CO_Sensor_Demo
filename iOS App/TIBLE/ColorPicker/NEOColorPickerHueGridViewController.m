...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.selectedColorLabel.text = NSLocalizedString(@"ColorPicker.SelectedColor.Label", nil);
    
    CGRect frame = CGRectMake(125, 5, 70, 35);
    ...
    checkeredView.backgroundColor = [UIColor clearColor];//[UIColor colorWithPatternImage:[UIImage imageNamed:@"colorPicker.bundle/color-picker-checkered"]];
    [self.view addSubview:checkeredView];
    
    CALayer *layer = [CALayer layer];
    layer.frame = CGRectMake(125, 5, 70, 35);
    ...
    for (int i = 0; i < 12; i++) {
        ...
        for (int x = 0; x < colorCount; x++) {
            ...
            int column = x % 4;
            int row = x / 4;
            layer.frame = CGRectMake(i * 320 + 8 + (column * 78), 8 + row * 43, 70, 35);
            [self setupShadow:layer];
            [self.scrollView.layer addSublayer:layer];
        }
    }

    self.scrollView.contentSize = CGSizeMake(3840, 250);
