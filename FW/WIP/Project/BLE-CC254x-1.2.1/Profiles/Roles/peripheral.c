    ...
    case GAP_LINK_ESTABLISHED_EVENT:
      {
        ...
        if ( pPkt->hdr.status == SUCCESS )
        {
          ...
          {
            GAPRole_SendUpdateParam( gapRole_MaxConnInterval, gapRole_SlaveLatency, gapRole_TimeoutMultiplier, GAPROLE_NO_ACTION );
          }
                ...
                || (pPkt->connTimeout != gapRole_TimeoutMultiplier) )
        {
          ...
          switch ( ParamUpdateNoSuccessOption )
          {
            case GAPROLE_RESEND_PARAM_UPDATE:
              GAPRole_SendUpdateParam( gapRole_MaxConnInterval, gapRole_SlaveLatency, gapRole_TimeoutMultiplier, GAPROLE_RESEND_PARAM_UPDATE );
              break;
